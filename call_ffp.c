...
FILE *ffp(int ac, int ar)
{
  ...
  sprintf(cmd, "ffplay -loglevel debug -i pipe:0 -f f32le -ac %d -ar %d", ac, ar);
...
{
  char cmd[1024];
  sprintf(cmd, "ffmpeg -hide_banner -loglevel panic -y -f f32le -ac 2 -ar 48000 -i pipe:0 -filter_complex 'showwavespic=s=640x120' -frames:v 1 %s", png_name); // ac, ar);
