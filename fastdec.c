...
int process(adsr_t* eg, double* ob, int n) {
  ...
  for (int i = 0; i < n; i++) {
    if (eg->delay-- > 0) {
      ...
      printf("%f attackend  %f\n", eg->egval, eg->att_rate_db);

    } else if (eg->att_rate_db > 0.000001f) {
      eg->dbval += eg->att_rate_db;
      eg->egval = pow(10.0, eg->dbval * 0.005);
      printf("%f attack %f %f\n", eg->dbval, eg->egval, eg->att_rate_db);
...
int main() {
  ...
  printf("sustan %hd rate %f sf2 %hd steps %f\n", z->VolEnvSustain,
         env->decay_rate, z->VolEnvDecay,
         96.0f / cent2samples(z->VolEnvDecay, 48000));
  printf("att %f hold %d\n", env->att_rate_db, env->hold);
  printf(" releae %f decay %f\n", env->release_rate, env->decay_rate);
  double ob[129];

  for (int i = 0; i < 2; i++) {
    if (process(env, ob, 128)) break;
  }
  printf("%d", z->VolEnvRelease);
