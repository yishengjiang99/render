...
function pcmPlayback(e) {
	...
	const zone = new Proxy(attrs, {
		get: (target, attr) => genstrs.indexOf(attr) > -1
			? target[genstrs.indexOf(attr)]
			: null
	})
	const rangeHeaders = {
		headers: {
	...
	const lpf = new BiquadFilterNode(ctx, {
		frequency: zone.InitialFilterFC,
		type: "lowpass"
	});
	abs.connect(lpf).connect(ctx.destination);
	abs.start();
}