...
int main(int argc, char **argv) {
  ...
  echo(
      ...
      "<body>"
      "<header> ");
  echo("<div id=inverse_fft_canvas_container></div>");

     echo(
         "</header>"
         "<script type='module' "
         "src='./dist/playsample.js'></script>\n\r<main><div>");
     for (int i = 0; i < 128; i++) {
       ...
       for (int z = 0; z < pz->npresets; z++) {
         ...
         fprintf(output, "<td>%hd</td><td>%hd</td><td>%hd</td>",
                 zones->Attenuation, zones->ModEnv2FilterFc,
                 zones->ModEnv2Pitch);
         ...
         fprintf(output, "<td>%hd</td><td>%hd</td><td>%hd</td><td>%hd</td>",
                 zones->ModLFO2Pitch, zones->VibLFO2Pitch,
                 zones->ModLFO2FilterFc, zones->ModLFO2Vol),
         ...
         for (int i = zones->KeyRange.lo;
           ...
           fprintf(
               output,
               "<a href='javascript:;' midi='%d' class='pcm'>%d%s</a>&nbsp;", i,
               i / 12, notestr[i % 12]);
         }
  ...
  echo(
      "</div>"
      "</main><footer></footer>"
      "<aside id='details'><div id=fft_canvas_container></div></aside>"
